    ...
    public class TstDictionary<T> : IDictionary<string, T>, ICollection, IReadOnlyDictionary<string, T>
    {
        ...
        public TstDictionary(IComparer<string> comparer)
        {
            ...
            if (Comparer == StringComparer.Ordinal)
                ...
                _compare = (x, y) => Comparer.Compare(x.ToString(), y.ToString());
        }
        ...
        /// <returns>
        ///     <c>true</c> if the <see cref="T:System.Collections.Generic.Dictionary`2" /> contains an element with the
        ...
        {
            var arr = values.OrderBy(v => v.Key, Comparer).ToArray();
                ...
                else
                {
                    if (p.Value.Key != null)
                    {
                        var res = p.Value.Key.Length == index + 1
                                  || Comparer.Compare(p.Value.Key, key) == 0;
                        entry = res ? p : null;
                        return res;
                    }
        ...
        private void AddRecursive(KeyValuePair<string, T>[] values, int start, int count)
        {
            switch (count)
            {
                ...
                default:
                    ...
                    for (var i = start + bucket; i < start + count - bucket; i++) Add(values[i]);
        ...
        protected sealed class TstDictionaryEnumerator<S> : IEnumerator<KeyValuePair<string, S>>
        {
            ...
            public bool MoveNext()
            {
                ...
                if (_stack == null)
                {
                    ...
                    _currentNode = null;
                    if (_dictionary._root != null) _stack.Push(_dictionary._root);
                }
        ...
        protected class TstDictionaryEntry<S>
        {
            ...
            /// </value>
            public bool IsKey => _value.Key != null;
            ...
            ///     The low child.
            /// </value>
            public TstDictionaryEntry<S> LowChild { get; set; }
            ...
            ///     The eq child.
            /// </value>
            public TstDictionaryEntry<S> EqChild { get; set; }
            ...
            ///     The high child.
            /// </value>
            public TstDictionaryEntry<S> HighChild { get; set; }
            ...
            /// </value>
            public bool HasChildren => LowChild != null || EqChild != null || HighChild != null;
        ...
        private static void VisitNode(TstDictionaryEntry<T> node, string path)
        {
            if (node == null)
                return;
            ...
            if (node.IsKey)
                Debug.Print(path + "'" + node.Value.Key + "' = " + node.Value);
